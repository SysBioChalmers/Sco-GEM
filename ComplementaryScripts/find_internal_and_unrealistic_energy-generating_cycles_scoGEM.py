...
scoGEM_FN = str(REPO_DIR / "ModelFiles" / "xml" / "scoGEM.xml")
SOLVER = "cplex"
...
def find_egc_standalone(cobra_model, already_irreversible=False):
    """
        try to locate EGCs by blocking all transport reactions and
        maximizing the objective of creating ATP from ADP + Pi
    """
...
if __name__ == '__main__':
    ...
    model = cobra.io.read_sbml_model(scoGEM_FN)
    model.solver = SOLVER
    find_egc_standalone(model, already_irreversible=False);
